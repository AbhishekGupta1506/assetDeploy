...
public class DeployTestTaskInboxResultsDefaultviewView extends com.webmethods.caf.faces.bean.task.BaseTaskInboxResultPageBean {
	...
	private com.webmethods.caf.deploytesttaskinboxresults.DeployTestTaskSearchProvider taskSearchProvider = null;

	private static final String[][] TASKSEARCHPROVIDER_PROPERTY_BINDINGS = new String[][] {
		{"#{DeployTestTaskSearchProvider.searchQuery.taskTypeID.value}", "708F0279-CDEC-A308-F018-E90393A483AB"},
		{"#{DeployTestTaskSearchProvider.rowIdBinding}", "#{row.taskInfo.taskID}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.taskID.value}", "#{activePageBean.activeSearchQuery.refineFields.taskID}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.priority.value}", "#{activePageBean.activeSearchQuery.refineFields.priority}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.createdDateRange}", "#{activePageBean.activeSearchQuery.refineFields.createdDateRange}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.modifiedDateRange}", "#{activePageBean.activeSearchQuery.refineFields.modifiedDateRange}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.expirationDateRange}", "#{activePageBean.activeSearchQuery.refineFields.expirationDateRange}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.acceptedByCurrent.value}", "#{activePageBean.activeSearchQuery.refineFields.acceptedByCurrentUser}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.invocationID}", "#{activePageBean.activeSearchQuery.invocationID}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.maxResults}", "#{activePageBean.activeSearchQuery.maxResults}"},
		{"#{DeployTestTaskSearchProvider.autoRefresh}", "#{not empty activePageBean.activeSearchQueryString}"},
		{"#{DeployTestTaskSearchProvider.searchQuery.invocationID}", "#{activePageBean.activeSearchQuery.invocationID}"},
	};
	...
	public com.webmethods.caf.deploytesttaskinboxresults.DeployTestTaskSearchProvider getTaskSearchProvider()  {
		if (taskSearchProvider == null) {
		    taskSearchProvider = (com.webmethods.caf.deploytesttaskinboxresults.DeployTestTaskSearchProvider)resolveExpression("#{DeployTestTaskSearchProvider}");
		}
